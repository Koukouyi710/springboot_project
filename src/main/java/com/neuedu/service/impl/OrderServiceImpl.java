...
public class OrderServiceImpl implements IOrderService{
    ...
    public ServerResponse pay(Integer userId, Long orderNo) {
        ...
        if (orderNo==null||orderNo.equals("")){
            return ServerResponse.createServerResponseByFail("支付的订单号不能为空！");
        }
        // 测试当面付2.0生成支付二维码
        Order order = orderMapper.findOrderListByUserIdAndOrderNO(userId,orderNo);
        if (order==null){
    ...
    public ServerResponse pay(Order order) throws IOException {
        ...
        AlipayTradePrecreateRequestBuilder builder = new AlipayTradePrecreateRequestBuilder()
                ...
                //.setNotifyUrl("http://gda6ej.natappfree.cc/order/alipay_callback.do")//支付宝服务器主动通知商户服务器里指定的页面http路径,根据需要设置
                .setNotifyUrl("http://47.96.237.127:8888/order/alipay_callback.do")
